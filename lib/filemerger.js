...
(function() {
  var async, config, crypto, fileMerger, fs, mkdirp, parser, path, tempFilesStatus, _;
  ...
  fileMerger = {
          ...
          if (merging) {
            _.each(mergeList, function(mergers) {
              ...
              _.each(mergers.files, function(file, i) {
                return content.push(fs.readFileSync(path.join(config.path, file), 'utf8'));
              });
    ...
    */
    ...
    mergeFilesToTemp: function(mergeFiles, type) {
      var getFileHash, linkFileHash, linkFileName, saveFile, self;
      self = this;
      ...
      if (tempFilesStatus[linkFileHash] === 'complete' || fs.existsSync(saveFile)) {
        ...
        if (!tempFilesStatus[linkFileHash]) {
          ...
          self.mergeFiles(mergeFiles, saveFile, function(data, file, saveFile) {
            var ext, imagesPath;
            ext = path.extname(file);
